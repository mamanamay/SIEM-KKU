<script>
    ...
    let lines = [];
    $: lastEvent = $eventsStore[0];
    
    onMount(() => {
      lines = ['[SYS] Initializing AI Threat Monitor...', '[SYS] Loading ML Models...', '[OK] Models loaded successfully.', '[SYS] Listening to real-time event stream...'];
      const interval = setInterval(() => {
        if(lastEvent) {
          lines = [...lines, `[ALERT] Anomaly detected from IP ${lastEvent.ip}`, `[ANALYSIS] Pattern matches ${lastEvent.type}`, `[ACTION] Recommending IP Block on Firewall.`].slice(-20);
        }
      }, 5000);
  ...
  <div class="page-container">
    <div class="page-title"><i class="ti ti-brain"></i> AI Threat Monitor</div>
    <div class="panel">
      <p style="font-size: 11px; color: var(--text-muted); margin-bottom: 10px;">* AI predictions are simulated based on real incoming honeypot events.</p>
