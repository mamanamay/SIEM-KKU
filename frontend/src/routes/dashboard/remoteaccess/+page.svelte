<script>
    ...
    $: remote = $eventsStore.filter(e => e.type.includes('SSH') || e.type.includes('RDP') || e.type.includes('Login'));
  </script>
  <div class="page-container">
    <div class="page-title"><i class="ti ti-devices-pc"></i> Remote Access Logs (VPN/SSH/RDP)</div>
    <div class="panel">
      <table class="data-table">
        <tr><th>Time</th><th>User/Account</th><th>Source IP</th><th>Protocol</th><th>Status</th></tr>
        {#each remote.slice(0, 10) as r}
        <tr>
          <td>{r.time || r.timeStr}</td>
          <td>root / admin</td>
          <td style="font-family: monospace;">{r.ip}</td>
          <td>{r.protocol || 'SSH'}</td>
          <td><span class="badge b-red">FAILED</span></td>
        </tr>
        {/each}
        {#if remote.length === 0}<tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--text-muted)">No remote access logs found.</td></tr>{/if}
      </table>
    </div>
  </div>
<style>
