<script>
    ...
    $: uniqueIps = [...new Set($eventsStore.map(e => e.ip))].slice(0, 10);
  </script>
  <div class="page-container">
    <div class="page-title"><i class="ti ti-virus"></i> Threat Intelligence</div>
    <div class="kpi-row">
      <div class="kpi" style="--kpi-color: var(--red)"><div class="kpi-label">Malicious IPs</div><div class="kpi-value">{uniqueIps.length}</div><div class="kpi-sub">Detected in last 24h</div></div>
      <div class="kpi" style="--kpi-color: var(--orange)"><div class="kpi-label">Known Actors</div><div class="kpi-value">2</div><div class="kpi-sub">AbuseIPDB matched</div></div>
    </div>
    <div class="panel">
      <p style="font-size: 11px; color: var(--text-muted); margin-bottom: 10px;">* Threat confidence scores are simulated for demo purposes based on real captured IPs.</p>
      <table class="data-table">
        <tr><th>Indicator (IP)</th><th>Confidence Score</th><th>Category</th><th>Source DB</th></tr>
        {#each uniqueIps as ip, i}
        <tr>
          <td style="font-family: monospace;">{ip}</td>
          <td>
            <div style="display:flex;align-items:center;gap:8px;">
              <div style="width:100px;height:6px;background:var(--border);border-radius:3px;overflow:hidden;"><div style="width:{95 - i*8}%;height:100%;background:var(--red);"></div></div>
              <span style="font-size:11px;font-weight:bold;color:var(--red)">{95 - i*8}%</span>
            </div>
          </td>
          <td><span class="badge b-orange">Brute-Force</span></td>
          <td>AbuseIPDB</td>
        </tr>
        {/each}
      </table>
